...
partition idx leftwall high pivotIdx arr = 
  let piv = arr !! pivotIdx
  in if idx == high  
        then swap pivotIdx leftwall arr ++ quick (take (leftwall - 1) arr) ++ quick (drop leftwall arr)
        else if (arr !! idx ) < (arr !! pivotIdx)
          then  partition (idx+1) (leftwall+1) high pivotIdx (swap idx leftwall arr) 
          else partition (idx+1) leftwall high pivotIdx arr
---partition :: Int -> Int -> Int -> Int-> [Int] -> [Int]
--partition idx leftwall high pivotIdx arr =
--  let piv = arr !! pivotIdx 
--  in case idx of  
...
quick :: [Int] -> [Int]
quick [] = []
quick arr = partition 0 0 (length arr - 1) 0 arr
